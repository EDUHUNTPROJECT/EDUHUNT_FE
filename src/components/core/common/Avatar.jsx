    ...
    if((session && session.user) || email && id != "null") {
        return (
            <div className="max-h-12 grid place-items-center" style={overflowStyle}>
                ...
                <div className="w-40 rounded text-xl boxshadow-parent-div"  style={{background: "white"}}>
                        ...
                        <div className="loginNavBar rounded m-3 pb-1 text-center boxshadow-child-div hov-bg-grey hov-text-white">
                            <button onClick={() => 
                                    {
                                        if(session && session.user) {signOut(); route.replace("http://localhost:3000/login");}
                                        else {localStorage.removeItem("userEmail"); localStorage.removeItem("userId"); route.replace("/")}
                                    }
                                }>Sign Out</button>
                        </div>
                </div>
