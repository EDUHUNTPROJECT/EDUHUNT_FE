...
export default function RegisterPage() {
  ...
  const submit = (e) => {
    ...
    registerUser({ name, email, password, confirmPassword })
      .then(() => {
        console.log("Add Acount Success");
      })
      .catch((error) => {
        alert(error.message);
      });
    ...
    <>
      <div className="bg-[#BEE9F2] min-h-screen flex items-center justify-center">
        <form onSubmit={submit}>
          <div className="bg-white rounded-lg p-8 w-[628px]">
            <div className="w-[470px] mx-auto">
              <h1 className="text-2xl font-semibold mb-4 text-center">
                Create your Account
              </h1>
              <div>
                <span className="text-[#333] text-sm">User name</span>
                <input
                  type="text"
              ...
              <div>
                <span className="text-[#333] text-sm">Confirm Password</span>
                <input
                  type="password"
              ...
              <p className="text-gray-600 mb-6 pt-[12px]">
                Already have a account?
                <Link href="/login">
                  <span className="text-[#12B8FF]"> Login</span>
                </Link>
              </p>
            </div>
          </div>
